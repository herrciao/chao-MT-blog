...
export async function generateMetadata({ params }: PostPageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `/en/posts/${params.slug}`,
      languages: {
        'zh-TW': `/posts/${params.slug}`,
...
export default function EnglishPostPage({ params }: PostPageProps) {
  const locale = 'en';
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="mb-8">
          <ol className="flex items-center space-x-2 text-sm text-gray-500">
            <li>
              <Link href="/en" className="hover:text-gray-700">
                {t.nav.home}
              </Link>
            </li>
            <li>/</li>
